...
namespace EneePrueba.Api.Modules.Factura.Features.ConsultarFacturas;
...
public static class Endpoint
{
    public static void ConsultarFacturas(this IEndpointRouteBuilder app)
    {
        app.MapGet(
                "/",
                async (IQueryDispatcher dispatcher, [AsParameters] FacturasFilters filters) =>
                {
                    var id = Guid.NewGuid();
                    IPaginated<MiFactura>? result = await dispatcher.Execute(
                        new ConsultarFacturaQuery(filters)
                    );
                    return result;
                }
            )
