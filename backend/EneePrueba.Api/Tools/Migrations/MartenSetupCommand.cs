...
namespace EneePrueba.Api.Tools.Migrations;
    ...
    public override Task<bool> Execute(GenerateCodeInput input)
    {
        using (IHost host = input.BuildHost())
        {
            ...
            if (!((IEnumerable<ICodeFileCollection>)array).Any<ICodeFileCollection>())
            {
                ...
                Console.Write(
                    "[red]No registered ICodeFileCollection services were detected, aborting.[/]"
                );
            ...
            dynamicCodeBuilder.WriteGeneratedCode(
                (Action<string>)(file => Console.WriteLine("Wrote generated code file to " + file))
            );
            Console.WriteLine("Run Test..");
            Console.WriteLine("Trying to generate all code and compile, this might take a bit.");
            dynamicCodeBuilder.TryBuildAndCompileAll(
                (Action<GeneratedAssembly, IServiceVariableSource>)(
                    (a, s) => new AssemblyGenerator().Compile(a, s)
                )
            );
