...
public class CrearFacturaCommandHandler(IWritableEventStoreRepository<Aggregates.Factura> facturas)
    : ICommandHandler<CrearFacturaCommand>
{
    public async Task Handle(CrearFacturaCommand command)
    {
        var factura = new Aggregates.Factura(
            command.Id,
            command.Numero,
